...
class Telegram:
    ...
    OWNER_ID = int(env.get('OWNER_ID', '7427294551'))
    WORKERS = int(env.get("WORKERS", "10"))  # 6 workers = 6 commands at once
    DATABASE_URL = str(env.get('DATABASE_URL'))
    UPDATES_CHANNEL = str(env.get('UPDATES_CHANNEL', "Ashlynn_Repository"))
    SESSION_NAME = str(env.get('SESSION_NAME', 'FileStream'))
    FORCE_SUB_ID = env.get('FORCE_SUB_ID', None)
    FORCE_SUB = env.get('FORCE_UPDATES_CHANNEL', False)
    FORCE_SUB = True if str(FORCE_SUB).lower() == "true" else False
    SLEEP_THRESHOLD = int(env.get("SLEEP_THRESHOLD", "60"))
    FILE_PIC = env.get('FILE_PIC', "https://ar-hosting.pages.dev/1733713261384.jpg")
    START_PIC = env.get('START_PIC', "https://ar-hosting.pages.dev/1733712663806.jpg")
